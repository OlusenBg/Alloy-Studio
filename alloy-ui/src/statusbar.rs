...
fn problems_chip(s: &StatusBarSignals, on_click: Arc<dyn Fn()>) -> impl View {
    ...
    chip(
        h_stack((
            label(|| "⊘".to_string()).style(|s| s.color(STATUS_ERROR).font_size(T_TINY).margin_right(4.0)),
            label(move || e.get().to_string()).style(|s| s.color(FG_2).font_size(T_TINY).margin_right(8.0)),
            label(|| "⚠".to_string()).style(|s| s.color(STATUS_WARNING).font_size(T_TINY).margin_right(4.0)),
...
fn cursor_chip(s: &StatusBarSignals) -> impl View {
    ...
    let c = s.cursor_col;
    container(
        label(move || format!("Ln {}, Col {}", l.get(), c.get()))
            .style(|s| s.color(FG_2).font_size(T_TINY).font_family("monospace".to_string())),
    )
