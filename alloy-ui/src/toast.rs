...
pub enum ToastTone { Info, Success, Warning, Error }
...
pub fn toast_overlay(
    ...
    .style(move |s| {
        let s = s
            ...
            .inset_right(12.0)
            .z_index(500);
        if toasts.get().is_empty() { s.hide() } else { s }
    })
}
        ...
        v_stack((
            h_stack((
                container(label(move || icon.to_string()).style(move |s| {
                    s.color(accent).font_size(T_TINY).font_weight(floem::text::Weight::BOLD)
                }))
                .style(move |s| {
                    s.width(18.0).height(18.0).border_radius(R_FULL)
                        .background(floem::peniko::Color::from_rgba8(
                            accent.r, accent.g, accent.b, 0x22))
                        .items_center().justify_center().flex_shrink(0.0)
                }),
                v_stack((
                    ...
                    label(move || body.clone()).style(|s| {
                        s.color(FG_3).font_size(T_TINY).margin_top(2.0)
                    }),
                ...
                container(label(|| "x".to_string()).style(|s| s.color(FG_3).font_size(T_TINY)))
                    ...
                    .style(|s| {
                        s.padding(4.0).cursor(CursorStyle::Pointer)
            ...
            if actions.is_empty() {
                ...
                h_stack((
                    floem::views::stack_from_iter(actions.into_iter().map(|a| {
                        ...
                        .style(|s| {
                            s.padding_horiz(10.0).padding_vert(4.0)
