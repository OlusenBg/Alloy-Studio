...
use floem::views::{Decorators, container, dyn_stack, empty, h_stack, label, scroll, text_input, v_stack};
...
fn cat_row(cat: SettingsCategory, active: RwSignal<SettingsCategory>) -> impl View {
        ...
        h_stack((
            ...
            container(empty()).style(move |s| {
                let s = s
                    ...
                    .border_radius(R_2)
                    .margin_left(0.0);
                if active.get() == cat { s } else { s.hide() }
            }),
            label(move || glyph.to_string()).style(move |s| {
                let c = if active.get() == cat { ALLOY_ORANGE } else { FG_2 };
    ...
    .style(move |s| {
        ...
        if active.get() == cat { s.background(BG_CURRENT) } else { s }
    })
}
...
fn right_content(active_cat: RwSignal<SettingsCategory>) -> impl View {
    scroll(
        container(
            v_stack((
                container(general_page()).style(move |s| if active_cat.get() == SettingsCategory::General { s } else { s.hide() }),
                container(editor_page()).style(move |s| if active_cat.get() == SettingsCategory::Editor { s } else { s.hide() }),
                container(appearance_page()).style(move |s| if active_cat.get() == SettingsCategory::Appearance { s } else { s.hide() }),
                container(ftc_page()).style(move |s| if active_cat.get() == SettingsCategory::Ftc { s } else { s.hide() }),
                container(ai_page()).style(move |s| if active_cat.get() == SettingsCategory::Ai { s } else { s.hide() }),
                container(git_page()).style(move |s| if active_cat.get() == SettingsCategory::Git { s } else { s.hide() }),
                container(extensions_page()).style(move |s| if active_cat.get() == SettingsCategory::Extensions { s } else { s.hide() }),
                container(keyboard_page()).style(move |s| if active_cat.get() == SettingsCategory::Keyboard { s } else { s.hide() }),
                container(about_page()).style(move |s| if active_cat.get() == SettingsCategory::About { s } else { s.hide() }),
            ))
            .style(|s| s.width_pct(100.0)),
        )
        .style(|s| s.max_width(720.0).padding(40.0).padding_top(32.0).padding_bottom(60.0)),
    )
...
fn page_h1(title: &'static str, sub: &'static str) -> impl View {
    v_stack((
        ...
        label(move || sub.to_string())
            .style(|s| s.color(FG_3).font_size(T_SMALL).margin_top(6.0)),
    ))
    ...
    h_stack((
        v_stack((
            label(move || label_text.to_string()).style(|s| {
                s.color(FG_1).font_size(T_SMALL).font_weight(floem::text::Weight::MEDIUM)
            }),
            label(move || hint.to_string()).style(move |s| {
                let s = s.color(FG_3).font_size(T_TINY).margin_top(3.0).line_height(1.45);
                if hint.is_empty() { s.hide() } else { s }
            }),
        ))
    ...
    text_input(sig)
        ...
        .style(move |s| {
            let s = s
                ...
                .font_size(T_SMALL)
                .border_radius(R_4);
            if mono { s.font_family("monospace".to_string()) } else { s }
        })
}
...
fn toggle(initial: bool, lbl: &'static str) -> impl View {
    ...
    h_stack((
        container(
            container(empty()).style(move |s| {
                let on = sig.get();
                let s = s.width(14.0).height(14.0).border_radius(R_FULL).background(floem::peniko::Color::WHITE);
                if on { s.margin_left(16.0) } else { s.margin_left(2.0) }
            }),
        )
...
fn general_page() -> impl View {
    v_stack((
        ...
        group("Workspace", v_stack((
            field("Auto-save", "Saves modified files after a delay.", toggle(true, "After 1s idle")),
            field("Restore on launch", "Reopen last project + tabs.", toggle(true, "Enabled")),
            field("Telemetry to Alloy", "Anonymous usage stats, off by default.", toggle(false, "Disabled")),
        )).style(|s| s.flex_col().gap(14.0))),
        group("Updates", v_stack((
            field("Update channel", "Pre-built binaries are not yet available — track main for now.",
...
fn editor_page() -> impl View {
    v_stack((
        ...
        group("Font", v_stack((
            field("Editor font family", "", select_pill("JetBrains Mono",
...
fn ftc_page() -> impl View {
    v_stack((
        page_h1("FTC", "Robotics-specific options. Surface only when an FTC project is detected."),
        group("Project", v_stack((
            field("Team number", "Shown in the title bar.", text_field("7842", false)),
            field("SDK version", "Detected from build.gradle.",
                  select_pill("9.2.0 (detected)", &["9.2.0 (detected)", "9.1.0", "9.0.1"])),
            field("Auto-detect FTC projects", "Look for FtcRobotController/ on open.",
                  toggle(true, "Enabled")),
        )).style(|s| s.flex_col().gap(14.0))),
        group("Robot", v_stack((
            field("Robot Controller IP", "Default is 192.168.43.1 (DS hotspot).",
                  text_field("192.168.43.1", true)),
            field("Telemetry UDP port",  "alloy-telemetry-bridge.apk listens here.",
                  text_field("9988", true)),
            field("Deploy method", "",
                  select_pill("adb push (USB)", &["adb push (USB)", "adb wifi", "OnBot Java"])),
        )).style(|s| s.flex_col().gap(14.0))),
    ))
    .style(|s| s.flex_col())
}
...
fn git_page() -> impl View {
    v_stack((
        ...
        group("Identity", v_stack((
            field("Name", "", text_field("Alex Chen", false)),
            field("Email", "", text_field("alex@7842.team", false)),
        )).style(|s| s.flex_col().gap(14.0))),
        group("Accounts", v_stack((
            field("GitHub", "Sign in to clone private repos and open pull requests.",
                  primary_btn_sm("⎇ Connect GitHub…")),
        )).style(|s| s.flex_col().gap(14.0))),
        group("Behaviour", v_stack((
            field("Default branch name", "", text_field("main", true)),
            field("Auto-fetch", "Pulls new commits from origin in the background.",
                  select_pill("every 5 minutes",
...
fn extensions_page() -> impl View {
    ...
    let cards = installed.into_iter().map(|(name, id, ver)| extension_card(name, id, ver));
    v_stack((
...
fn extension_card(name: &'static str, id: &'static str, ver: &'static str) -> impl View {
    h_stack((
        container(label(|| "⬚".to_string())
            .style(|s| s.color(ALLOY_ORANGE).font_size(T_XL)))
            .style(|s| {
                s.width(32.0).height(32.0)
                    .border_radius(R_4)
        ...
        v_stack((
            ...
            label(move || format!("{id} · v{ver}")).style(|s| {
                s.color(FG_3).font_size(T_TINY).font_family("monospace".to_string())
            }),
        ))
...
fn keyboard_page() -> impl View {
    ...
    let rows = shortcuts.into_iter().enumerate().map(|(i, (lbl, kbd))| {
        ...
        h_stack((
            label(move || lbl.to_string()).style(|s| s.color(FG_1).font_size(T_SMALL).flex_grow(1.0)),
        ...
        .style(move |s| {
            let s = s.padding_horiz(14.0).padding_vert(10.0).items_center().font_size(T_SMALL);
            if !is_last { s.border_bottom(1.0).border_color(BG_EDGE) } else { s }
        })
    });
    v_stack((
        page_h1("Keyboard", "Default bindings. Custom keymaps coming in beta."),
        container(floem::views::stack_from_iter(rows).style(|s| s.flex_col().width_pct(100.0)))
            .style(|s| {
                s.background(BG_SURFACE).border_radius(R_6).width_pct(100.0)
            }),
    ))
    .style(|s| s.flex_col())
}
    ...
    v_stack((
        v_stack((
            floem::views::img(|| ALLOY_LOGO.to_vec())
                .style(|s| s.width(80.0).height(80.0)),
        ...
        label(|| "Forked from Lapce · Apache-2.0 · Brand marks © respective owners".to_string())
            .style(|s| s.color(FG_4).font_size(T_MICRO).margin_top(24.0).width_pct(100.0)),
    ))
...
fn resources_grid() -> impl View {
    ...
    let rows = items.into_iter().map(|(lbl, url)| {
        h_stack((
            label(|| "→".to_string()).style(|s| s.color(ALLOY_ORANGE).font_size(T_BASE).margin_right(10.0)),
            v_stack((
    ...
    floem::views::stack_from_iter(rows)
        .style(|s| s.flex_col().gap(6.0).width_pct(100.0))
}

fn primary_btn_sm(text: &'static str) -> impl View {
    container(label(move || text.to_string()).style(|s| s.color(FG_1).font_size(T_TINY).font_weight(floem::text::Weight::SEMIBOLD)))
        .style(|s| {
            s.padding_horiz(12.0)
                .padding_vert(6.0)
                .background(ALLOY_ORANGE)
