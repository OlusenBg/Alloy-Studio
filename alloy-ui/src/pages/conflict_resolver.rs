...
pub enum Side { Yours, Theirs }

#[derive(Clone, Copy, PartialEq)]
pub enum SynTok { Keyword, Function, String, Number, Type, Variable, Builtin, Punct }
...
pub fn conflict_resolver(
    conflicts: RwSignal<Vec<ConflictFile>>,
    h: ConflictHandlers,
) -> impl View {
    v_stack((
...
fn warning_banner(
    ...
    h_stack((
        label(|| "!".to_string()).style(|s| s.color(ALLOY_ORANGE).font_size(T_LG).margin_right(12.0)),
        v_stack((
            label(move || format!(
                "Merge conflict in {} files — Alloy will help you resolve them.",
                conflicts.get().len()
            )).style(|s| s.color(FG_1).font_size(T_BASE).font_weight(floem::text::Weight::BOLD)),
            label(move || {
                let total = conflicts.get().len();
                let resolved = conflicts.get().iter().filter(|c| c.resolution.is_some()).count();
    ...
    .style(|s| {
        s.padding_horiz(20.0).padding_vert(12.0)
...
fn conflict_card(f: ConflictFile, h: ConflictHandlers) -> impl View {
    ...
    v_stack((
        h_stack((
            label(move || name.clone()).style(|s| {
                s.color(FG_1).font_size(T_BASE).font_weight(floem::text::Weight::SEMIBOLD)
            }),
            label(move || path.clone()).style(|s| {
                s.color(FG_3).font_size(T_TINY)
                    .font_family("monospace".to_string()).margin_left(8.0)
            }),
            container(empty()).style(|s| s.flex_grow(1.0f32)),
            status_pill(resolution),
        ))
        .style(|s| {
            s.padding_horiz(16.0).padding_vert(12.0)
                .background(BG_RAISED).border_bottom(1.0).border_color(BG_EDGE)
                .items_center().gap(10.0).width_pct(100.0)
        }),
        h_stack((
            label(|| "*".to_string()).style(|s| {
                s.color(ALLOY_ORANGE).font_size(T_BASE).margin_top(2.0).margin_right(10.0)
            }),
            label(move || summary.clone()).style(|s| {
                s.color(FG_2).font_size(T_SMALL).flex_grow(1.0)
            }),
        ))
        .style(|s| {
            s.padding_horiz(16.0).padding_vert(10.0)
                .background(BG_SURFACE)
                .items_start().gap(10.0).width_pct(100.0)
        }),
        h_stack((
            side_view("Your version", STATUS_INFO, f.yours.clone(),
                      Side::Yours, resolution == Some(Side::Yours),
                      f.name.clone(), h.on_keep.clone()),
            container(empty()).style(|s| s.width(1.0).background(BG_EDGE).height_pct(100.0)),
            side_view("Their version", STATUS_WARNING, f.theirs.clone(),
                      Side::Theirs, resolution == Some(Side::Theirs),
                      f.name.clone(), h.on_keep.clone()),
        ))
        .style(|s| s.width_pct(100.0)),
        container(
            container(label(|| "Open in editor".to_string())
                .style(|s| s.color(FG_1).font_size(T_TINY)))
                .on_click_stop({
                    let n = f.name.clone();
        ...
        .style(|s| {
            s.padding(10.0).background(BG_SURFACE)
    ...
    .style(move |s| {
        s.background(BG_SURFACE)
            .border_radius(R_8)
            .border_left(3.0)
            .border_color(if resolved { STATUS_SUCCESS } else { STATUS_ERROR })
            .width_pct(100.0)
            .flex_col()
    })
}
...
fn status_pill(resolution: Option<Side>) -> impl View {
    h_stack((
        label(move || match resolution {
            ...
            let c = if resolution.is_some() { STATUS_SUCCESS } else { STATUS_ERROR };
        ...
        label(move || match resolution {
            ...
            let c = if resolution.is_some() { STATUS_SUCCESS } else { STATUS_ERROR };
            s.color(c).font_size(T_TINY).font_weight(floem::text::Weight::SEMIBOLD)
        }),
    ))
...
fn side_view(
    ...
    let lbl = match s_kind { Side::Yours => "yours", Side::Theirs => "theirs" };

    v_stack((
        h_stack((
            container(empty()).style(move |s| {
                s.width(6.0).height(6.0).background(accent).border_radius(R_2)
            }),
            label(move || title.to_uppercase()).style(move |s| {
                s.color(accent).font_size(T_MICRO)
                    .font_weight(floem::text::Weight::BOLD)
                    .margin_left(8.0)
            }),
            label(move || format!(" - {} - {}", author, when)).style(|s| {
                s.color(FG_3).font_size(T_MICRO)
            }),
        )).style(|s| s.items_center()),
        label(move || explain.clone()).style(|s| s.color(FG_2).font_size(T_TINY)),
        v_stack((
            dyn_stack(
                move || code.iter().enumerate().map(|(i, l)| (i, l.clone())).collect::<Vec<_>>(),
                |(i, _)| *i,
                |(_, line)| {
                    h_stack((
                        dyn_stack(
                            move || line.iter().enumerate().map(|(i, t)| (i, t.clone())).collect::<Vec<_>>(),
                            |(i, _)| *i,
                            |(_, (tok, text))| {
                                label(move || text.clone()).style(move |s| {
                                    s.color(tok.color()).font_size(T_SMALL)
            ...
            ).style(|s| s.flex_col().padding_horiz(10.0).padding_vert(8.0).width_pct(100.0)),
        ))
        .style(|s| {
        ...
        container(label(move || {
            if chosen { format!("Kept {lbl}") } else { format!("Keep {lbl}") }
            ...
            .style(move |s| {
                let s = s
                    .padding_horiz(12.0).padding_vert(6.0)
                    .border_radius(R_4)
                    .cursor(CursorStyle::Pointer);
                if chosen {
...
fn footer_bar(
    ...
    h_stack((
        label(move || {
            ...
            if unresolved == 0 {
                ...
                format!("{unresolved} conflict{} left.",
                    if unresolved == 1 { "" } else { "s" })
            }
        }).style(|s| s.color(FG_3).font_size(T_SMALL)),
        container(empty()).style(|s| s.flex_grow(1.0f32)),
        container(label(|| "Discard changes".to_string())
            .style(|s| s.color(FG_1).font_size(T_TINY)))
    ...
    .style(|s| {
        s.items_center().gap(8.0)
            .padding(16.0).margin_top(4.0)
            .border_top(1.0).border_color(BG_EDGE)
            .width_pct(100.0)
    })
}
